...
function I(a) {
  try {
    const t = a?.data || {},
      e = t.marker || {},
      r = Array.isArray(t.matrix)
        ? t.matrix.slice(0, 16)
        : t.matrix && t.matrix.length
          ? Array.from(t.matrix).slice(0, 16)
          : null,
      o = e.vertex
        ? Array.isArray(e.vertex)
          ? e.vertex.slice()
          : null
        : e.corners
          ? e.corners.flatMap((i) => [i.x ?? i[0], i.y ?? i[1]])
          : null;
    return {
      type: t.type,
...
function W(a) {
  const t = a?.data || {},
    e = t.type,
    r = t.marker || {},
    o = r.idPatt ?? r.patternId ?? r.pattern_id ?? null,
    i = r.cfPatt ?? r.confidence ?? 0,
    n = t.matrix;
...
function v() {
  !s ||
    ...
    T ||
    (s.addEventListener('getMarker', (a) => {
      if (!W(a)) return;
      const t = I(a);
      try {
...
async function U(a = 640, t = 480) {
  ...
  k = (async () => {
    try {
      const r = await (async () =>
          l.moduleUrl
            ? (console.log('[Worker] Loading artoolkit from moduleUrl:', l.moduleUrl),
              await import(l.moduleUrl))
            : await import('./ARToolkit-pmDcsFY0.js').then(function (c) {
                return c.A;
              }))(),
        o = r.ARController ?? r.default?.ARController,
        i = r.ARToolkit ?? r.default?.ARToolkit;
      if (!o) throw new Error('ARController export not found in ARToolKit module');
      ...
      if (
        ...
        (s = await o.initWithDimensions(a, t, n, {})),
        (d = !!s),
        console.log('[Worker] ARToolKit initialized:', d),
        !d)
      )
        throw new Error('ARController.initWithDimensions returned falsy controller');
      (v(), (h = 0), (M = 0));
    } catch (r) {
      (console.error('[Worker] ARToolKit init failed:', r),
        (s = null),
        (d = !1),
        (h = Math.min(h + 1, 6)));
      ...
      throw (
        ...
        f({
          type: 'error',
          payload: { message: `ARToolKit init failed (${r?.message || r}). Retrying in ${o}ms.` },
        }),
        r
      );
    } finally {
...
E(async (a) => {
  ...
  try {
    if (t === 'init') {
      (e &&
        typeof e == 'object' &&
        ((l.moduleUrl = e.moduleUrl ?? l.moduleUrl),
        (l.cameraParametersUrl = e.cameraParametersUrl ?? l.cameraParametersUrl),
        (l.wasmBaseUrl = e.wasmBaseUrl ?? l.wasmBaseUrl),
        typeof e.minConfidence == 'number' &&
          ((l.minConfidence = e.minConfidence), (g = e.minConfidence))),
        A || (f({ type: 'ready' }), (A = !0)));
      return;
    }
    if (t === 'loadMarker') {
      ...
      if (!r) {
        f({
          ...
          payload: { ok: !1, error: 'Missing patternUrl parameter', requestId: i },
        });
        return;
      }
      try {
        ...
        const c = await F(r);
        (typeof s.trackPatternMarkerId == 'function'
          ? s.trackPatternMarkerId(c, o)
          : typeof s.trackPatternMarker == 'function' && s.trackPatternMarker(c, o),
          f({ type: 'loadMarkerResult', payload: { ok: !0, markerId: c, size: o, requestId: i } }));
      } catch (n) {
        (console.error('[Worker] loadMarker error:', n),
          f({
            type: 'loadMarkerResult',
    ...
    if (t === 'processFrame') {
      ...
      if (r) {
        try {
          const n = o || r.width || 640,
            c = i || r.height || 480;
          (await U(n, c),
            (!p || u !== n || m !== c) &&
              ((u = n),
              (m = c),
              (p = new OffscreenCanvas(u, m)),
              (w = p.getContext('2d', { willReadFrequently: !0 }))),
            w.clearRect(0, 0, u, m),
            w.drawImage(r, 0, 0, u, m));
          try {
            r.close?.();
          } catch {}
          if (d && s)
